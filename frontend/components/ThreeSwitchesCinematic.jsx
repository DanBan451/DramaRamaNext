 ...
 * ----------------------
 * ~8s cinematic intro for the "Three Switches, Two Rooms, and One Bulb" puzzle.
 * Uses the cinematic-hallway.webm as the full-screen base layer; all text/overlay
...
// ─── Main component ───────────────────────────────────────────────────────────
export default function ThreeSwitchesCinematic({
  sessionId,
  puzzleText = "",
  onComplete,
  speed = 1,
  ...
  // Fire onComplete at 14s (longer puzzle text) or on skip
  useEffect(() => {
    if (completed) return;
    if (skipped || t >= 14000) {
      setCompleted(true);
  ...
  return (
    <div className="fixed inset-0 z-30 overflow-hidden pointer-events-none bg-black">
      ...
      >
        <source src="/cinematic-hallway.webm" type="video/webm" />
      </motion.video>
